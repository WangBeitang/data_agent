...
def test_query_table_converts_decimal_to_float():
    """MySQL SUM 等返回 Decimal，必须转换为 JsonScalar（float）。"""
    table = QueryTable(
        columns=["销售额"],
        rows=[{"销售额": Decimal("109030.5")}],
        row_count=1,
    )
    assert table.rows == [{"销售额": 109030.5}]
    assert isinstance(table.rows[0]["销售额"], float)
